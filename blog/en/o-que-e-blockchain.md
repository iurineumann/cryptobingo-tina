...
**"Blockchain consumes enormous energy."** Not all blockchains are Bitcoin. WAX has been carbon neutral since 2021. The energy per transaction is negligible compared to proof-of-work chains.
...
For gaming in particular, blockchain eliminates the fundamental trust problem: players no longer need to trust that the platform is honest. The code guarantees honesty.
...
1. **Provably fair draws:** every draw uses a cryptographic random number generator with three independent entropy sources. After each game, anyone can verify that the result was not manipulated. See our [provably fair guide](/blog/provably-fair) for the technical details.
